...
Spinner A will have higher probability of penny. There are 6 coins in total for both spinner, spinner A has 4 pennies and 2 nickels, and spinner B has 3 pennies and 3 nickels. Thus, the probability of penny for spinner A is 4/6 which is greater than spinner B's 3/6.

<br>

**State new a LOWER bound for your estimate for Spinner A, and explain your reasoning:**

The lower bound for penny win by using spinner A is 1/2, because the number of penny on spinner A is more than a half.

<br>

**State new a UPPER bound for your estimate for Spinner A, and explain your reasoning:**

The upper bound for penny win is 2/3 because only 4 pennies out of 6 coins, so the probability is 4/6.
...
```{r}
Spinner_B <- c("Penny", "Penny", "Penny", "Nickel", "Nickel", "Nickel")
nsim <-1000
pennyWins <- rep(NA, nsim)
for(i in 1:nsim){
  pennyWins[i]<-sample(Spinner_B, 1)
  outcomes <-array(pennyWins[i])
  wins <-0
  while(sum(outcomes == "Penny") < 5 & sum(outcomes =="Nickel") < 5){
    if(pennyWins[i] == "Penny"){
      wins <- wins+1
    } 
  }
  print(outcomes/nsim)
...
```{r}
Spinner_A <- c("Penny", "Penny", "Penny", "Penny", "Nickel", "Nickel")
nsim <-1000
pennyWins <- rep(NA, nsim)
for(i in 1:nsim){
  pennyWins[i]<-sample(Spinner_A, 1)
  outcomes <-array(pennyWins[i])
  wins <-0
  while(sum(outcomes == "Penny") < 5 & sum(outcomes =="Nickel") < 5){
    if(pennyWins[i] == "Penny"){
